## Система такси сервиса.

## 1. Описание проекта
 Приложение для автоматизации работы службы такси.

---

## 2. Цель и задачи проекта
**Цель:** Автоматизация операционных процессов службы такси.

**Задачи:**
- Управление поездками
- Динамический расчёт стоимости
- Управление водителями
- Финансовые операции

---

## 3. Функциональность
- Ввод адреса
- Выбор тарифа
- История поездок
- Список водителей
- Отзывы
- Оплата

---

## 4. Архитектура проекта
Тип архитектуры: Многослойная (Layered / N-Tier)

**Технологии:**
- Visual Studio 2022
- Wpf
- C#
- SQL Server Management Studio
- MSTest
- Entity Framework

---

## 5. Демонстрация

**Ввод адреса**

<img width="790" height="492" alt="image" src="https://github.com/user-attachments/assets/957196cd-28f9-414e-a2a1-c5eb587f46be" />



---

**Выбор тарифа**

<img width="790" height="492" alt="image" src="https://github.com/user-attachments/assets/870e5e28-f0ac-4919-8b7b-d11d833e7ac6" />



---

**История поездок**

<img width="790" height="492" alt="image" src="https://github.com/user-attachments/assets/97a9b48b-0777-426b-8144-a3201ec6e649" />



---

**Список водителей**

<img width="790" height="492" alt="image" src="https://github.com/user-attachments/assets/b0d7f0ae-8de0-4236-a610-1824220a2189" />


---

**Отзывы**

<img width="790" height="492" alt="image" src="https://github.com/user-attachments/assets/ed06cebd-6ecc-4d27-b121-b457b88c5acb" />



---

**Страница оплаты**

<img width="790" height="492" alt="image" src="https://github.com/user-attachments/assets/b9c03333-9834-4ae6-a6af-0c2f68b927c2" />




---

## 6. Тестирование

**Результаты тестов:**

<img width="657" height="535" alt="image" src="https://github.com/user-attachments/assets/c740a06a-9570-4f53-a300-810ba3030f84" />



---

## 7. Результаты и выводы
В ходе выполнения проекта было разработано приложение для автоматизации работы службы такси.
